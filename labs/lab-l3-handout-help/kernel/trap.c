...
void usertrap(void)
{
    ...
    else if (r_scause() == 15)
    {
        ...
        // Page table entry is valid, accessible by the user, it is a COW page and it is currently already at leas readable or executable.
        if ((*pte & PTE_V) && (*pte & PTE_U) && (*pte & PTE_COW) && (*pte & (PTE_R | PTE_X)))
        {
            uint flags = PTE_FLAGS(*pte);
            // new page will not be cow, but writeable
            flags |= PTE_W;
            flags &= (~PTE_COW);
        ...
        {
            // It trapped in a case where it should not (as long as the user process behaves.)
            printf("SEGFAULT");
            setkilled(p);
        }
    }
