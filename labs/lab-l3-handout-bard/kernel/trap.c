...
extern char references[NUM_PPAGES];
  ...
  if(r_scause() == 8){
    ...
    *pte = (*pte >> 1) << 1;
    ...
    if(references[ppn] > 1){
      ...
      // increment references to new page
      references[PTE2PPN(*walk(pagetable, va, 0))] ++;
    } else {
      // There is only one reference to the page, so we can just change the permission
      *pte = *pte | PTE_W;
    }
